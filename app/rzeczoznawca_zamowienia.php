...
if ((!isset($_COOKIE['Bruk&Trawnik_pracownik_login'])) or ($_COOKIE['Bruk&Trawnik_pracownik_typ'] != $appraiser)) {
	...
	if ($_COOKIE['Bruk&Trawnik_pracownik_hasło'] != $password) {
		...
		$orders = pg_query($link, "SELECT orders.ID, CLIENT_ID, ORDER_DATE FROM orders WHERE APPRAISER = '$ID' ORDER BY ID DESC;");
...
?>
		...
		<html>
			<head>
				<meta charset='utf-8'>
				<title>Panel pracownika - zamówienia oczekujące na Twoją wycenę</title>
			</head>
			<body>
			<?php
			if ($ile == 0) {
				echo "<b>Brak zamówień oczekujących na Twoją wycenę.</b><br><br>";
			}

			else {
			?>
				<b><u>Zamówienia oczekujące na Twoją wycenę</u></b><br><br>
				<table>
					<tr>
					...
					<?php
						while ($order=pg_fetch_row($orders)) {
							echo "<tr>
									...
									<td><form action='zamowienie_szczegoly_rzeczoznawca.php' method='POST'>
